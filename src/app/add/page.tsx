...
import { PRODUCT_TAXONOMY } from "@/features/resources/categorize-resource";
import { parseBookmarkHtml } from "@/features/resources/adapters/bookmark-import";
import type { CategorizationResult, NormalizedCandidate, ResourceContentType } from "@/features/resources/types";
...
export default function AddResourcePage() {
  ...
  async function confirmSave() {
    ...
    const categories = PRODUCT_TAXONOMY.filter((category) => selectedCategories.has(category.slug)).map((category) => ({
      slug: category.slug,
      label: category.label,
      confidence: preview.suggestions.categories.find((c) => c.slug === category.slug)?.confidence ?? 0.6,
    }));
    ...
    try {
      const response = await fetch("/api/resources", {
        ...
        body: JSON.stringify({
          ...
          summary: preview.suggestions.summary,
          thumbnailUrl: preview.candidate.thumbnailUrl,
          contentType,
          estimatedMinutes,
          durationSeconds: preview.candidate.durationSeconds,
          durationIsEstimated: preview.candidate.durationIsEstimated,
          categories: categories.length > 0 ? categories : undefined,
          goalMatches: goalMatches.length > 0 ? goalMatches : undefined,
          ...
          {stage === "confirmation" && preview && (
            <>
              ...
              <div className="mt-4 grid grid-cols-2 gap-4">
                ...
                <label className="block text-sm font-medium text-[var(--muted)]">
                  Estimated minutes {preview.candidate.durationIsEstimated ? "(estimated)" : ""}
                  <input
                    className="mt-2 min-h-12 w-full rounded-2xl border border-[var(--border)] px-4 font-normal text-[var(--foreground)]"
                    min={1}
